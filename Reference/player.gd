...
#grab sound node
@onready var get_foot_sound = get_node("audio_pos/sound_emitter")
...
func check_floor():
	...
	for area in areas:
		...
		if area.is_in_group("floors") and is_on_floor():
			print(ground_snd_type)
			ground_snd_type = area.get_ground_type()
...
func get_walk_sound(current_step):
	...
	if is_on_floor():
		...
		#var get_foot_sound = sound_path.format({"type": ground_snd_type})
